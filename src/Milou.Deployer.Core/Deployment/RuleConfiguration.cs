...
namespace Milou.Deployer.Core.Deployment
{
    public class RuleConfiguration
    {
        ...
        public static RuleConfiguration Get(DeploymentExecutionDefinitionV1 deploymentExecutionDefinition,
            ...
            ILogger logger)
        {
            ...
            bool doNotDeleteEnabled = deploymentExecutionDefinition.DoNotDeleteEnabled(
                deployerConfiguration.WebDeploy.Rules.DoNotDeleteRuleEnabled);

            bool useChecksumEnabled = deploymentExecutionDefinition.UseChecksumEnabled(
                deployerConfiguration.WebDeploy.Rules.UseChecksumRuleEnabled);

            bool appDataSkipDirectiveEnabled = deploymentExecutionDefinition.AppDataSkipDirectiveEnabled(
                deployerConfiguration.WebDeploy.Rules.AppDataSkipDirectiveEnabled);

            bool applicationInsightsProfiler2SkipDirectiveEnabled =
                deploymentExecutionDefinition.ApplicationInsightsProfiler2SkipDirectiveEnabled(deployerConfiguration
                   .WebDeploy.Rules.ApplicationInsightsProfiler2SkipDirectiveEnabled);

            bool appOfflineEnabled = deploymentExecutionDefinition.AppOfflineEnabled(
                deployerConfiguration.WebDeploy.Rules.AppOfflineRuleEnabled);

            bool whatIfEnabled = deploymentExecutionDefinition.WhatIfEnabled();

            logger.Debug("{RuleName}: {DoNotDeleteEnabled}",
                nameof(deployerConfiguration.WebDeploy.Rules.DoNotDeleteRuleEnabled),
                doNotDeleteEnabled);

            logger.Debug("{RuleName}: {AppOfflineEnabled}",
                nameof(deployerConfiguration.WebDeploy.Rules.AppOfflineRuleEnabled),
                appOfflineEnabled);

            logger.Debug("{RuleName}: {UseChecksumEnabled}",
                nameof(deployerConfiguration.WebDeploy.Rules.UseChecksumRuleEnabled),
                useChecksumEnabled);

            logger.Debug("{RuleName}: {AppDataSkipDirectiveEnabled}",
                nameof(deployerConfiguration.WebDeploy.Rules.AppDataSkipDirectiveEnabled),
                appDataSkipDirectiveEnabled);

            logger.Debug("{RuleName}: {ApplicationInsightsProfiler2SkipDirectiveEnabled}",
            ...
            logger.Debug("{RuleName}: {WhatIfEnabled}",
